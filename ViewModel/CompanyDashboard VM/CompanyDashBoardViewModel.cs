using Scalaverse.Entitys.Models;
using System;
using System.Collections.Generic;

namespace ScalaVerse.ViewModel.Company_Dashboard_VM
{
    public class CompanyDashboardViewModel
    {
        // Statistics
        public int TotalProjects { get; set; }
        public int ActiveProjects { get; set; }
        public int CompletedProjects { get; set; }
        public decimal TotalRevenue { get; set; }
        public decimal AverageRating { get; set; }

        // Incoming Requests
        public int PendingRequestsCount { get; set; }
        public List<CompanyRequestSummary> IncomingRequests { get; set; }

        // Recent Projects
        public List<CompanyProjectSummary> RecentProjects { get; set; }

        // Company Info
        public string CompanyName { get; set; }
        public string ManagerName { get; set; }
        public RegisteredCompany Company { get; set; }

        public CompanyDashboardViewModel()
        {
            IncomingRequests = new List<CompanyRequestSummary>();
            RecentProjects = new List<CompanyProjectSummary>();
        }

        public string WelcomeMessage
        {
            get
            {
                var hour = DateTime.Now.Hour;
                var greeting = hour < 12 ? "Good Morning" : hour < 18 ? "Good Afternoon" : "Good Evening";
                return $"{greeting}, {ManagerName}!";
            }
        }
    }

    // Request summary
    public class CompanyRequestSummary
    {
        public int RequestId { get; set; }
        public string ProjectName { get; set; }
        public string ClientName { get; set; }
        public string Description { get; set; }
        public decimal? ClientBudget { get; set; }
        public string Status { get; set; }
        public DateTime CreatedAt { get; set; }
        public string SelectedServiceName { get; set; }
        public string SelectedServiceTier { get; set; }

        public string TimeAgo
        {
            get
            {
                var span = DateTime.UtcNow - CreatedAt;
                if (span.TotalMinutes < 60) return $"{(int)span.TotalMinutes} minutes ago";
                if (span.TotalHours < 24) return $"{(int)span.TotalHours} hours ago";
                return $"{(int)span.TotalDays} days ago";
            }
        }
    }

    // Project summary
    public class CompanyProjectSummary
    {
        public int ProjectId { get; set; }
        public string ProjectName { get; set; }
        public string ClientName { get; set; }
        public string Status { get; set; }
        public int ProgressPercentage { get; set; }
        public decimal Budget { get; set; }

        public string StatusBadgeClass
        {
            get
            {
                return Status switch
                {
                    "Completed" => "badge bg-success",
                    "Active" => "badge bg-primary",
                    "InProgress" => "badge bg-info",
                    "OnHold" => "badge bg-warning",
                    _ => "badge bg-secondary"
                };
            }
        }
    }
}